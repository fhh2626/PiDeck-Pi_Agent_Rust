...
**一个同时支持原版 [Pi Agent](https://pi.dev) 和 Pi_Agent_Rust、用于管理多个编码 Agent 会话的桌面工作台。**
...
**PiDeck** 是一个开源的桌面工作台，同时支持原版 [Pi Agent](https://pi.dev) 和 Pi_Agent_Rust，用于在本地项目目录中统一管理编码 Agent 会话，并支持导入 Codex、Claude 本地会话以便统一浏览和恢复。基于 Electron + TypeScript 构建，提供多项目工作区、AI 会话管理、Git 集成、内置终端、模型配置和插件扩展能力，让本地 AI 编码助手在多项目环境中保持统一、可追溯、可配置。

**适合谁用：** 希望在桌面端同时管理多个本地项目的原版 Pi Agent 或 Pi_Agent_Rust 编码会话、需要统一查看会话历史与 Git 状态、并希望以图形化方式管理 Agent 运行时配置的开发者。

`PiDeck` **不是**原版 Pi Agent 或 Pi_Agent_Rust 的分支。它是一个轻量 Electron 外壳，通过启动兼容的 RPC Agent 进程，将项目管理、会话管理、对话界面、配置管理和工具编排整合到一个原生桌面应用中——具体的 Agent 能力由所选运行时原生提供。
...
| 功能 | 说明 |
|---|---|
| **多项目工作区** | 添加、搜索、拖动排序和切换本地项目目录，同时运行来自原版 Pi Agent 或 Pi_Agent_Rust 的多个 Agent，项目间完全隔离。 |
...
- Node.js 20+
- npm
- 系统 `PATH` 中可访问所选 Agent 运行时
- 已完成所选 Agent 运行时的 Provider / 登录 / API Key 配置

请根据所选 Agent 运行时的安装说明验证其是否可用。以原版 Pi Agent 为例：
...
> PiDeck 需要单独安装受支持的 Agent 运行时，并确保其加入系统 `PATH`。
...
本应用启动本地 `pi` 进程并通过 Electron IPC 暴露有限的文件操作。请仅运行你信任的源码。PiDeck 不发送匿名使用统计或其他遥测数据；不会收集项目路径、代码、消息内容、会话内容或文件名，也不会上传文件。pi agent 子进程代理和桌面端模型拉取/测试代理可独立配置；系统浏览器打开的外部链接仍由系统浏览器网络设置决定。
